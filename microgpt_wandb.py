"""
01_microgpt.py - Karpathy's microgpt 完全解説版（日本語名前データセット + wandb）
...
このファイルは microgpt.py の全コードに日本語の解説コメントを付けたものです。
...
# コマンドライン引数（ハイパーパラメータ）
# =============================================================================
# uv run microgpt_wandb.py                          # デフォルト設定で学習
# uv run microgpt_wandb.py --n_embd 64 --n_head 8   # 埋め込み次元・ヘッド数を変更
...
# 推論スクリプト（generate.py）でこの重みを読み込んで名前を生成できる。
